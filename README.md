...
```
app 默认调 `qwen-plus`（`chat_model` 可改）。

**无 Key 占位 —— mock**：起 mock 并把 base 指向它
```bash
uv run uvicorn scripts.mock_llm_server:app --port 4000   # .env: DASHSCOPE_BASE_URL=http://localhost:4000
```
工具路由 / 规则检索 / SSE / 落库全真，仅 LLM 文本是占位。
