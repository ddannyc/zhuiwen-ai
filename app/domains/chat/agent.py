...
async def prepare(
    ...
    一次独立的可流式调用。多轮工具沿用 MAX_TOOL_ROUNDS。

    返回 {action, tools_used, needs_gen, gen_messages, static_reply}：
      - needs_gen=True → service 用 gen_messages 调 chat / chat_stream 生成终答（可流式）。
      - needs_gen=False → 事前定的静态回复（当前仅空检索），用 static_reply，不生成不流。
    """
    tools = TOOLS  # 已含 answer 标记工具
    ...
    for rnd in range(MAX_TOOL_ROUNDS):
        if force_tool and rnd == 0:
            tool_choice: Any = {"type": "function", "function": {"name": force_tool}}
        else:
            tool_choice = "required"  # 必选某工具（含 answer）→ 路由不自由生成正文
        route_msg = await chat_with_tools(msgs, tools=tools, model=model, tool_choice=tool_choice)
        calls = route_msg.get("tool_calls") or []
        real = [c for c in calls if c["function"]["name"] != "answer"]
        if not real:
            # 路由决定直接答（answer 或未调真工具）→ 终答就绪。answer 路由 msg 不入 gen_messages。
