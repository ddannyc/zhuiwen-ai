...
class SourcingRepository:
    ...
    async def requeue_if_stale(self, batch_id: str, running_grace_seconds: int) -> bool:
        """cron 兜底原子重投：仅当批仍掉队（pending/queued，或 running 超 running_grace）时
        重置为 queued + 刷新 updated_at，并返回 True。否则（如刚被 worker 认领为 running、
        updated_at 新）不动、返回 False。

        与「先 find 再无条件 set」相比，这里把 staleness 判断和重置合进一条 UPDATE，
        闭合 TOCTOU 竞态——不会把刚被认领的 running 打回 queued 致同批双跑（review #1）。"""
        uid = _as_uuid(batch_id)
        if uid is None:
            return False
        running_cutoff = _now() - timedelta(seconds=running_grace_seconds)
        res = await self.session.execute(
            update(CollectJob)
            .where(
                CollectJob.id == uid,
                or_(
                    CollectJob.post_status.in_([POST_PENDING, POST_QUEUED]),
                    and_(
                        CollectJob.post_status == POST_RUNNING,
                        CollectJob.updated_at < running_cutoff,
                    ),
                ),
            )
            .values(post_status=POST_QUEUED, updated_at=_now())
            .returning(CollectJob.id)
        )
        await self.session.flush()
        return res.first() is not None
