...
def _fuse(
    ...
    # 两个排名表（用 rule_id 作 key——自解释且稳，不依赖对象身份）。
    vec_rank = {r["rule_id"]: i for i, r in enumerate(sorted(rows, key=lambda r: r["dist"]))}
    lex_sorted = sorted([r for r in rows if r["_lex"] > 0], key=lambda r: -r["_lex"])
    lex_rank = {r["rule_id"]: i for i, r in enumerate(lex_sorted)}

    def rrf(r: dict[str, Any]) -> float:
        s = 1.0 / (_RRF_K + vec_rank[r["rule_id"]])
        if r["rule_id"] in lex_rank:
            s += 1.0 / (_RRF_K + lex_rank[r["rule_id"]])
        return s
...
class RulesKbService:
    ...
    async def search(
        ...
        try:
            ...
            # 不静默吞：DB 不可达 或 _search_db/融合 里任何 bug 都该留痕，否则 prod 会
            # 无声降级到劣质 jsonl 路径、毫无信号。保留降级，但 warning 记录。
            _log.warning("rules_kb DB 检索失败，回退 jsonl：%s", e)
            return _search_jsonl(query, platform=platform, site=site, limit=limit)
        # 空结果：区分"表空"（→回退 jsonl）与"语义无关"（→正确返空，守反幻觉）。
        # is_empty 仅在无命中时触发（罕见路径），开销可忽略。
